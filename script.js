...
let receiptLogoUrl = localStorage.getItem('receiptLogoUrl') || '';
let cart = [];
...
let currentUser = null; // Ini akan menyimpan pengguna yang sedang login

// DOM Elements
// --- AWAL PERBAIKAN PENTING DOM ELEMENTS ---
const loginModal = document.getElementById('loginModal'); // *** Diperbaiki: Disesuaikan dengan ID di HTML Anda ***
const loginUsernameInput = document.getElementById('login-username'); // *** Diperbaiki: Disesuaikan dengan ID di HTML Anda ***
const loginPasswordInput = document.getElementById('login-password'); // *** Diperbaiki: Disesuaikan dengan ID di HTML Anda ***
const loginBtn = document.getElementById('login-btn'); // Ini sudah benar
const loginCompanyNameDisplay = document.getElementById('login-company-name-display'); // Ini sudah benar
// --- AKHIR PERBAIKAN PENTING DOM ELEMENTS ---
...
const cartItemsContainer = document.querySelector('.cart-items'); // Menggunakan querySelector untuk class
...
const receiptLogo = document.getElementById('receipt-logo'); // Element untuk logo struk
...
let incomeChart = null; // Variabel untuk menyimpan instance chart
...
function saveData() {
    ...
    localStorage.setItem('receiptLogoUrl', receiptLogoUrl);
    ...
    // *** Perbaikan: Simpan juga currentUser agar sesi login bertahan ***
...
function handleLogin() {
    ...
    if (user) {
        currentUser = user;
        saveData(); // *** Perbaikan: Simpan sesi login pengguna saat ini setelah login berhasil ***
        loginModal.classList.remove('active'); // Hanya hapus kelas 'active'

        showNotification(`Selamat datang, ${currentUser.username}!`, 'success');
        updateTabVisibility();

        // Aktifkan tab pertama yang sesuai dengan peran user
        ...
        if (activeTabButton && activeTabButton.style.display !== 'none') {
            ...
            // Fallback jika tidak ada tab yang visible, aktifkan kasir saja
            document.querySelector('.tab-button[data-tab="kasir"]').classList.add('active');
...
function updateTabVisibility() {
    tabButtons.forEach(button => {
        ...
        // Jika peran 'kasir' dan tab adalah produk, pengeluaran, rekapan, atau pengaturan
        if (currentUser && currentUser.role === 'kasir' && (tabId === 'produk' || tabId === 'pengeluaran' || tabId === 'rekapan' || tabId === 'pengaturan')) {
            button.style.display = 'none'; // Sembunyikan tombol tab
            // Jika tab yang disembunyikan sedang aktif, alihkan ke tab Kasir
            if (button.classList.contains('active')) {
                document.querySelector('.tab-button[data-tab="kasir"]').click();
            }
        } else {
            button.style.display = ''; // Tampilkan tombol tab
        }
    });
}
...
addToCartBtn.addEventListener('click', () => {
    ...
    // --- PERBAIKAN LOGIKA STOK SAAT MENAMBAHKAN KE KERANJANG ---
    const existingCartItem = cart.find(item => item.id === productId);
    const currentCartQuantity = existingCartItem ? existingCartItem.quantity : 0;
    const availableStock = product.stock - currentCartQuantity; // Stok yang tersedia (fisik - yang sudah di keranjang)

    if (quantity > availableStock) {
        showNotification(`Stok ${product.name} tidak cukup. Sisa stok: ${product.stock} (di keranjang: ${currentCartQuantity})`, 'error');
        return;
    }
    ...
    if (existingCartItem) {
        ...
        cart.push({
            ...
            name: product.name,
            price: product.price,
            costPrice: product.costPrice, // Pastikan costPrice juga ikut disimpan di item keranjang
            quantity: quantity
        });
    }
    ...
    if (e.target.closest('button')) {
        ...
        if (action === 'remove') {
            ...
            if (itemIndex > -1) {
                ...
                // Tidak perlu mengembalikan stok di sini karena stok fisik belum dikurangi
...
processPaymentBtn.addEventListener('click', () => {
    ...
    // --- PENGURANGAN STOK FISIK SAAT PEMBAYARAN DIPROSES ---
    ...
    const newTransaction = {
        ...
        items: JSON.parse(JSON.stringify(cart)), // Salin item keranjang untuk transaksi
    ...
    clearCart(); // Kosongkan keranjang setelah transaksi
    updateRecapSummary();
    updateChart();
    renderTransactionList();
    clearKasirInputs();
...
function updateRecapSummary() {
    ...
    transactions.forEach(transaction => {
        ...
        transaction.items.forEach(item => {
            // Pastikan item.costPrice ada dan valid
            totalCostOfGoodsSold += item.quantity * (item.costPrice || 0);
        });
    });
    ...
    transactions.forEach(transaction => {
        ...
        switch (filter) {
            ...
            case 'weekly':
                ...
                startOfWeek.setDate(transactionDate.getDate() - (dayOfWeek === 0 ? 6 : dayOfWeek - 1));
...
exportRecapPdfBtn.addEventListener('click', () => {
    ...
    transactions.forEach(transaction => {
        totalRevenue += transaction.total;
        transaction.items.forEach(item => {
            totalCostOfGoodsSold += item.quantity * (item.costPrice || 0); // Pastikan costPrice dihitung
        });
    });
...
clearAllDataBtn.addEventListener('click', () => {
    ...
    if (confirm('PERINGATAN: Tindakan ini akan menghapus SEMUA data aplikasi secara permanen! Anda yakin ingin melanjutkan?')) {
        ...
        currentUser = null; // Reset user setelah semua data dihapus
        showNotification('Semua data aplikasi telah dihapus. Harap login kembali.', 'success');
        loginModal.classList.add('active'); // Tampilkan kembali modal login
        updateUI(); // Refresh UI setelah clearing data
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LOGIKA INITIAL LOAD DAN LOGIN YANG DIPERBAIKI ---
    // Coba muat currentUser dari localStorage untuk mempertahankan sesi
    currentUser = JSON.parse(localStorage.getItem('currentUser'));

    if (!currentUser) {
        // Jika tidak ada user yang login, tampilkan modal login
        loginModal.classList.add('active');
        // Pastikan input login direset saat modal muncul
        loginUsernameInput.value = '';
        loginPasswordInput.value = '';
    } else {
        // Jika ada user yang login, sembunyikan modal login
        loginModal.classList.remove('active');
        // Atur visibilitas tab berdasarkan peran user yang login
        updateTabVisibility();
        // Klik tab pertama yang aktif atau tab Kasir jika tidak ada yang aktif/visible
        const activeTabButton = document.querySelector('.tab-button.active');
        if (activeTabButton && activeTabButton.style.display !== 'none') {
            activeTabButton.click();
        } else if (document.querySelector('.tab-button[data-tab="kasir"]').style.display !== 'none') {
            ...
            // Fallback: Jika kasir juga tidak terlihat (misal karena ada bug pada logika peran),
            // pastikan setidaknya tab kasir menjadi aktif secara default.
            document.querySelector('.tab-button[data-tab="kasir"]').classList.add('active');
            document.getElementById('kasir').classList.add('active');
        }
    }
    // Panggil updateUI untuk pertama kali setelah menentukan state login
    updateUI();
    // --- AKHIR LOGIKA INITIAL LOAD DAN LOGIN YANG DIPERBAIKI ---
});
...
// --- FITUR GESER (SCROLLABLE CONTAINER) ---
