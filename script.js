...
function populateProductSelect() {
    ...
    clearKasirInputs(); // Clear other inputs when product select is repopulated
}
...
function renderTransactionList() {
    ...
    const filterDate = reportDateFilter.value; // YYYY-MM format
    ...
    switch (filter) {
        ...
        case 'weekly':
            ...
            for (let i = 11; i >= 0; i--) {
                ...
                startOfWeek.setDate(d.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1)); // Adjust to Monday for consistency
    ...
    transactions.forEach(transaction => {
        ...
        switch (filter) {
            ...
            case 'weekly':
                ...
                startOfWeek.setDate(transactionDate.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // If there's an active tab (e.g., after login), ensure its content is rendered
    const activeTabButton = document.querySelector('.tab-button.active');
    if (activeTabButton) {
        const activeTabId = activeTabButton.dataset.tab;
        document.getElementById(activeTabId).classList.add('active');
        if (activeTabId === 'rekapan') {
            updateRecapSummary();
            updateChart();
        }
    }
